...
def get_5fold_test_probs(all_embs, all_emb_ways, labels, save_path, if_save_pred = False, task_name = '', seed_bias = 0):
    ...
    for time in range(10):
        ...
        for k, (tr_idx, tt_idx) in enumerate(kf.split(labels)):
            ...
            if save_path_name not in os.listdir('/'.join(save_path.split('/')[:-1])):
                ...
                if seed_bias != 0:
                    save_txt(f'{save_path}_time{time}_fold{k}_seed_bia{seed_bias}.txt', each_embs_probs)
                else:
...
def get_ind_test_probs(train_embs, test_embs, all_emb_ways, tr_labels, tt_labels, save_path, if_save_pred = False, task_name = ''):
    os.makedirs('/'.join(save_path.split('/')[:-1]), exist_ok = True)
...
def FS_ATP(train_data_file_name, test_data_file_name, pad_size, dataset_name):
    tr_embs, tt_embs, emb_ways, tr_labels, tt_labels = get_ind_embs_set(train_data_file_name, test_data_file_name, pad_size, device, dataset_name)
    # only w2v
    train_embs = tr_embs[:10]
    test_embs = tt_embs[:10]
...
def BS_ATP(train_data_file_name, test_data_file_name, pad_size, dataset_name):
    tr_embs, tt_embs, emb_ways, tr_labels, tt_labels = get_ind_embs_set(train_data_file_name, test_data_file_name, pad_size, device, dataset_name)
    # only w2v
    train_embs = tr_embs[:10]
    test_embs = tt_embs[:10]
    ...
    # test_embs = tt_embs
    # all_emb_ways = emb_ways
    # task_name = f'{dataset_name}_BS_all'
...
def BS_keep_or_drop_or_concat(train_data_file_name, test_data_file_name, pad_size, dataset_name):
    ...
    while len(all_emb_ways) > 1:
        ...
        if action_id == 0:
            ...
            return ['+'.join(all_emb_ways)] + sub_emb_ways
        elif action_id == 1:
            # drop
...
def fast_ensemble2(train_data_file_name, test_data_file_name, pad_size, dataset_name):
    ...
    for e_list in ensemble_list:
        ...
        # _, t = get_5fold_test_probs(curr_train_embs, curr_emb_ways, tr_labels, f'results/probs_txt/keep_or_concat/{task_name}_{e_list["dataset_name"]}_emb_{"_".join(curr_emb_ways)}', True, task_name + '_concat')
...
if __name__ == '__main__':
    ...
    print('forward selection concat')
    print(FS_list)
    print('backward selection concat')
    print(BS_list)
